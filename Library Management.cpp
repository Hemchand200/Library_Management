	...
	else
	{
		while(intf.read(reinterpret_cast<char *>(&bk),sizeof(book)))
		{
			if(bk.retbno()==x)
			{
				...
				bk.createb();
				bk.showb();
				intf.seekp(intf.tellp()-static_cast<std::streamoff>(sizeof(book)));
	...
	else
	{
		while(intf.read(reinterpret_cast<char *>(&st),sizeof(student)))
		{
			if(st.admno==x)
			{
				...
				st.creates();
				st.shows();
				intf.seekp(intf.tellp()-static_cast<std::streamoff>(sizeof(student)));
	...
	else
	{
		while(outf.read(reinterpret_cast<char *>(&st),sizeof(student)))
		{
			if(st.admno==sn)
			{
				...
				{
					st.settoken(bn);
					outf.seekp(outf.tellp()-static_cast<std::streamoff>(sizeof(student)));
					outf.write(reinterpret_cast<char *>(&st),sizeof(student));
					cout<<"\n\t\tBook Issued";
